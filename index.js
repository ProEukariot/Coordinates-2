...
function movingAverageFilter(data, windowSize) {
	...
	return data.map((_, i, arr) =>
		kernel.reduce(
			(acc, cur, k) =>
				acc + (arr[i - k] || 0) * cur * Number(!muteFiltered),
			0
		)
	);
}
...
function windowSize(data, size) {
	const filter = movingAverageFilter(data, size);

	const shift = signal.slice(size / 2);

	const MSE =
		shift.reduce((acc, val, i) => {
			const diff = val - filter[i];
...
for (let i = 0; i < 20; i++) {
	const MSE = windowSize(signalPlusNoise, i);

	console.log(`MSE for ${i}: ${MSE}`);
}
